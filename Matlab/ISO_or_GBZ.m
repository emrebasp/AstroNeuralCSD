...
% Parameter range for varied connection weights gamma_ei, gamma_ii
nOfSamples = 55; 
initRange = 0;
finRange = 10;
parRange = linspace(initRange,finRange,nOfSamples);
...
% Perform the simulation for each gamma_ei, gamma_cii
for i=1:nOfSamples
    
    % Keep track of the simulations
    fprintf('Iteration %d out of %d\n', i, nOfSamples);

    % Vary gamma_ei, gamma_ii
    modelp.gamma_ei = parRange(i);
    modelp.gamma_ii = parRange(i);
...
figure;
plot(parRange, propagationSpeed, 'bo','MarkerFaceColor', 'b', 'LineWidth', 2);
axis([0 parRange(end)+0.5 0 6])
% hold on;
% plot(par1,prop_vel_A,'--o','LineWidth',2);
grid off;
% legend('excitatory','astrocytes');
xlabel('\gamma_{ei}, \gamma_{ii}');
ylabel('Speed');
...
fileName = sprintf('GBZ_or_ISO_%s.png', timestamp);
...
% Define the filename with the timestamp
fileName = sprintf('GBZ_or_ISO_%s.csv', timestamp);

% Full path to save the file
fullFilePath = fullfile(savePath, fileName);

% Write data to the CSV file
data = [parRange' propagationSpeed']; % Combine data into a matrix (column-wise)
